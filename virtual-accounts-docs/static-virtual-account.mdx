...
sequenceDiagram
    ...
    A-->>-M: 2. 200 OK (Status: PROCESSING)
    Note over A,M: Account details sent via Webhook
    A->>M: 3. Webhook: virtualaccount.creation.completed
    M->>C: 4. Share Account Number
...
| `data.account.number` | String | Account number customers will fund (received via webhook). |
...
| `data.account.number` | String | Account number customers will fund (received via webhook). |
| `data.status` | String | Account status (e.g., `PROCESSING`, `ACTIVE`). |
| `data.currency` | String | ISO 4217 currency code (`MWK`). |
| `data.country` | String | ISO 3166-1 alpha-2 country code (`MW`). |
| `data.created` | String | ISO 8601 timestamp when the account was created. |
...
| `data.account.number` | String | Account number customers will fund (received via webhook). |
| `data.account.swiftCode` | String | SWIFT code for international transfers (received via webhook). |
| `data.status` | String | Account status (e.g., `PROCESSING`, `ACTIVE`). |
...
- **Monitor Creation Webhook**: Subscribe to `virtualaccount.creation.completed` to receive account details (Account Number, Bank Name, etc.) automatically.
- **Store the reference** and response payload so you can reconcile the virtual account with your internal records.
- **Mask the BVN** and other PII when logging requests to comply with data-protection policies.
- **Pair this endpoint** with [`Get Virtual Account`](/virtual-accounts-docs/get-virtual-account) to confirm account details if you need them before the webhook arrives.
- **Monitor Deposits**: Subscribe to `virtualaccount.completed` webhooks to reconcile deposits the moment they land.
