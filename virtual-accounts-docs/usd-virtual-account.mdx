...
sequenceDiagram
    ...
    M->>C: 4. Share Account Details & SWIFT Code
    loop International Deposits
        C->>+B: 5. Sends USD via SWIFT
        B->>+A: 6. SWIFT network notifies meCash
        A->>M: 7. Webhook: virtualaccount.completed
        M->>C: 8. Confirm receipt
    end
```
...
<Note>
**Account Details via Webhook**: For USD virtual accounts, the actual account number, bank name, and SWIFT details are delivered asynchronously. Once the account is ready, meCash will send a `virtualaccount.creation.completed` webhook to your configured endpoint.

For more details on the payload, see the [Virtual Account Creation Webhook](/webhook/virtualaccount-creation-webhook) documentation.
</Note>
