# CNY Payout – WECHAT (MOBILE)

This guide explains how to initiate a **CNY payout to a WeChat user via mobile number** using the meCash API. The request must be sent as `multipart/form-data` with the transaction payload provided as a stringified JSON object under a `transaction` field.
...
## Example Request (WeChat via MOBILE)
...
<Tabs items={['cURL', 'JavaScript (fetch)', 'Python (requests)', 'Node.js (axios)']}>
  ...
  <Tabs.Tab>
    ...
    .then(console.log)
    .catch(error => console.error('Error creating WeChat payout (Mobile):', error));
    ```
  </Tabs.Tab>
  ...
  <Tabs.Tab>
    ...
    .catch(error => {
      console.error('Error creating WeChat payout (Mobile):', error.response ? error.response.data : error.message);
    });
    ```
  </Tabs.Tab>
