---
title: 'Bulk Payouts'
description: 'Disburse funds to multiple beneficiaries in a single request for payroll, vendor payments, and mass disbursements.'
icon: users
tag: new
---
...
<Tip>
  **Environment Note:** The endpoints in this guide use the Sandbox URL (`https://sandboxapi.me-cash.com`). In Production, replace `sandboxapi` with `api` (`https://api.me-cash.com`).
...
<Tabs>
  <Tab title="Request">
    ```bash cURL
    curl --location 'https://sandboxapi.me-cash.com/v1/bulk/payout' \
...
<Tabs>
  <Tab title="Request">
    ```bash cURL
    curl --location 'https://sandboxapi.me-cash.com/v1/bulk/payout/{{batchId}}/status' \
    --header 'Content-Type: application/json' \
    --header 'x-api-key: YOUR_API_KEY'
    ```
  </Tab>
