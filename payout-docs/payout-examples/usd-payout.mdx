...
| `data.quote.paymentChannel`        | String | The payment channel used (e.g., `SWIFT_OUR`, `SWIFT_SHARED`, `SWIFT_BENEFICIARY`). |
...
✅ Use `SWIFT_OUR` when the sender bears all transfer charges, `SWIFT_SHARED` when charges are split, or `SWIFT_BENEFICIARY` when the beneficiary bears all charges.  
