...
If the quote expires or fails validation, the [payout endpoint](/payout-docs/create-payout) returns an error. Refresh the quote and retry the payout with the new `quoteId`.
...
<CardGroup cols={2}>
  ...
  <Card title="Bulk Quotes" icon="layer-group" href="/quote-docs/bulk-quote">
    Need to quote multiple transfers at once? Use the Bulk Quote API.
  </Card>
  <Card title="Create Payout" icon="paper-plane" href="/payout-docs/create-payout">
    Use your `quoteId` to disburse funds to a recipient.
  </Card>
</CardGroup>
