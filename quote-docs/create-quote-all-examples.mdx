...
<Note>Use `SWIFT_OURS` when the sender bears all charges, or `SWIFT_SHARED` when charges are split with the beneficiary. Both channels use the same quote request format.</Note>
