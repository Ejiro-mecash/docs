...
<Note>Use `SWIFT_OUR` when the sender bears all charges, `SWIFT_SHARED` when charges are split, or `SWIFT_BENEFICIARY` when the beneficiary bears all charges. All channels use the same quote request format.</Note>
