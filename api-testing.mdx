...
```bash
curl -X POST "https://api.me-cash.com/v1/quotes" \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "from_currency": "USD",
...
```bash
curl -X POST "https://api.me-cash.com/v1/payouts" \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "amount": "100.00",
...
```bash
export MECASH_API_KEY="your_api_key_here"
export MECASH_BASE_URL="https://api.me-cash.com"
```

Then use in your requests:
```bash
curl -X GET "$MECASH_BASE_URL/v1/wallets" \
  -H "x-api-key: $MECASH_API_KEY"
```
