...
<Accordion title="August 2026 — Version v1.x (Status: Live)">
  **New Features**

  - **Wallet capabilities**: Added support for USD wallet capabilities.
  - **Instant wallets**: Wallet availability can now indicate whether a wallet is instant.

  **Enhancements**

  - **Duplicate transactions**: Improved transaction handling to prevent duplicate transactions.
  - **CAD recipients**: Improved CAD recipient creation for both Bank Transfer and SWIFT beneficiaries.
  - **Bank country**: Improved destination bank country handling for SWIFT and Bank Transfer transactions.
  - **POBO**: Standardized "Myself" sender information across payout routes.
  - **Account name**: Improved account name handling so the account name provided in a request is displayed correctly.
  - **Error responses**: Improved error messaging and validation for clearer API responses.

  **Bug Fixes**

  - Fixed an issue with sender address information that could cause payout requests to fail.
  - Fixed an issue that could prevent USD SWIFT payouts from being submitted when using "Others" as the sender information.
  - Fixed an issue where wallet transaction data was not being returned correctly.
  - Fixed an issue where transaction values were not displaying correctly in API reports.
</Accordion>

<Accordion title="July 2026 — Version v1.x (Status: Live)">
  **New Features**

  - **Wallet capabilities**: Added API support for USD wallet capabilities.
  - **Multi-currency payments**: Improved handling of incoming foreign-currency payments into USD master accounts, automatically routing funds to the appropriate currency wallet.

  **Enhancements**

  - **Destination country**: Added destination country information to recipient creation for supported international transfers.
  - **Account Number / IBAN**: Recipient creation now accepts either an account number or IBAN, providing greater flexibility for international payouts.
  - **Dynamic payout forms**: Introduced a more flexible payout form structure that supports different recipient requirements across payout routes.
  - **EUR payouts**: Updated EUR payout fields to provide a more consistent experience with existing payout routes.
  - **API response codes**: Improved API responses with standardized response codes for more consistent error handling.
  - **Wallet details**: Improved wallet account-detail handling for supported currencies.
...
<Accordion title="June 2026 — Version v1.x (Status: Live)">
  **New Features**
...
<AccordionGroup>
  <Accordion title="May 2026 — Version v1.x (Status: Live)">
    **New Features**
    ...
    | :---------- | :---------------- | :-------------- |
    | `SWIFT_OUR` | Sender | The sending customer bears all transfer charges. |
    ...
    **Backward Compatibility**
    ...
    - Integrations still sending `paymentChannel: "SWIFT"` should migrate to `SWIFT_OUR`, `SWIFT_SHARED`, or `SWIFT_BENEFICIARY` at the earliest opportunity.
    - Choose `SWIFT_OUR` if your platform absorbs the full cost of the transfer.
    ...
    **Action needed:** Update your payout request payloads to use `SWIFT_OUR`, `SWIFT_SHARED`, or `SWIFT_BENEFICIARY` in place of the deprecated `SWIFT` value for USD Transactions. No other fields change — recipient account details, sender details, and quote request format remain identical.
  </Accordion>

  <Accordion title="March 2026 — Version v1.x (Status: Live)">
    **New Features**
  ...
  <Accordion title="February 2026 — Version v1.x (Status: Live)">
